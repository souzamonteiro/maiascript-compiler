...
#ifndef PRINTXQUERY_HPP_
#define PRINTXQUERY_HPP_
...
class PrintXQuery : public PrintVisitor
{
public:
  PrintXQuery(char **av,
    ...
    whitespaceCalled(false)
  {
    ...
    fileName = Format::newFileName(fName, ".xquery");
    free(fName);
  ...
  {
  public:
    PrintLoops(PrintXQuery *p) : px(p), loopId(0) {}
  ...
  const wchar_t *extension() const {return L".xquery";}
...
protected:
  ...
  void visitGrammar(Grammar *node)
  {
    ...
    OutputFile::printHeader(file, XQUERY, argv, true, grammar->useGlr ? "3.1" : "1.0");
    lineNo += 4;
    ...
    fputs(encoded, file);
    free(encoded);
    OutputFile::printFooter(file, XQUERY);
    fclose(file);
  }

  void invalidateCommentTokens(WString &w)
  {
    for (size_t found; WString::npos != (found = w.find(L"(:"));) w.replace(found, 2, L"(.");
    for (size_t found; WString::npos != (found = w.find(L":)"));) w.replace(found, 2, L".)");

#if 0
    enum {START, PARENTHESIS, COLON} state = START;
    for (wchar_t *p = w.c_str(); *p; ++p)
    {
      switch (state)
      {
      case START:
             if (*p == L'(') state = PARENTHESIS;
        else if (*p == L':') state = COLON;
        break;
      case PARENTHESIS:
        if (*p == L':')
        {
